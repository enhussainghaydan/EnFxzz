...
const MAX_CARD_IMAGE_LENGTH = 7 * 1024 * 1024;
...
async function readJson(file, fallback = []) {
  ...
  try {
    ...
    console.error(`Could not read ${path.basename(file)}. Keeping the file unchanged.`, error);
    return structuredClone(fallback);
  }
}
...
function isSafeImageDataUrl(value = '') {
  ...
  if (text.length > MAX_CARD_IMAGE_LENGTH) return false;
  return /^data:image\/(?:png|jpe?g|webp|gif);base64,[A-Za-z0-9+/=]+$/i.test(text);
}
...
async function ensureAuth(req, res, next) {
  ...
  if (storedUser.isActive === false) {
    ...
    req.session.destroy(() => {});
    return fail(res, 'معطل بسبب مخالفتك للقوانين أو تنهاء مدة الاشتراك', 403);
  }
...
async function ensureAdmin(req, res, next) {
  ...
  if (storedUser.isActive === false) {
    activeSessions.delete(req.sessionID);
    req.session.destroy(() => {});
    return fail(res, 'هذا  معطل بسبب مخالفتك للقوانين أو تنهاء مدة الاشتراك', 403);
  }
  req.session.user = sanitizeUser(storedUser);
...
app.post('/api/admin/cards', ensureAdmin, async (req, res) => {
  ...
  const image = clampText(req.body.image, MAX_CARD_IMAGE_LENGTH);
...
app.put('/api/admin/cards/:id', ensureAdmin, async (req, res) => {
  ...
  current.image = clampText(req.body.image || current.image, MAX_CARD_IMAGE_LENGTH);
  ...
  if (!isSafeCardImage(current.image)) {
    return fail(res, 'رابط صورة البطاقة غير صالح', 422);
  }
...
app.listen(PORT, HOST, () => {
  console.log(` Fxzz Store running on http://${HOST}:${PORT}`);
});

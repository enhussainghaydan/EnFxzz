...
const MAX_LOCAL_IMAGE_SIZE = 4 * 1024 * 1024;

function cardImagePickerMarkup({ value = '', inputName = 'image', sourceName = 'imageSource' } = {}) {
  const hasValue = Boolean(value);
  return `
    <div class="card-image-picker" data-card-image-picker>
      <div class="image-source-tabs">
        <label>
          <input type="radio" name="${escapeHtml(sourceName)}" value="file" />
          <span>صورة من الجهاز</span>
        </label>
        <label>
          <input type="radio" name="${escapeHtml(sourceName)}" value="url" checked />
          <span>رابط صورة</span>
        </label>
      </div>
      <div class="image-source-panel" data-image-panel="url">
        <input name="${escapeHtml(inputName)}" value="${escapeHtml(value)}" placeholder="https://..." />
      </div>
      <div class="image-source-panel hidden" data-image-panel="file">
        <input name="${escapeHtml(inputName)}File" type="file" accept="image/png,image/jpeg,image/webp,image/gif" />
        <small class="muted">اختر صورة PNG أو JPG أو WEBP أو GIF من جهازك.</small>
      </div>
      <div class="card-image-preview ${hasValue ? '' : 'hidden'}">
        <img src="${escapeHtml(value)}" alt="معاينة الصورة" />
      </div>
    </div>
  `;
}
...
function readImageFileAsDataUrl(file) {
  return new Promise((resolve, reject) => {
    ...
    if (file.size > MAX_LOCAL_IMAGE_SIZE) {
      reject(new Error('حجم الصورة كبير، اختر صورة أقل من 4MB'));
      return;
    }
...
async function getCardImageValue(form, fallback = '') {
  const source = form.querySelector('input[name="imageSource"]:checked')?.value || 'url';
  if (source === 'file') {
    const file = form.querySelector('input[name="imageFile"]')?.files?.[0];
    if (!file) return fallback;
    return readImageFileAsDataUrl(file);
  }
  return form.querySelector('input[name="image"]')?.value.trim() || fallback;
}

function setupCardImagePickers(scope = document) {
  scope.querySelectorAll('[data-card-image-picker]').forEach((picker) => {
    const urlPanel = picker.querySelector('[data-image-panel="url"]');
    const filePanel = picker.querySelector('[data-image-panel="file"]');
    const preview = picker.querySelector('.card-image-preview');
    const previewImg = picker.querySelector('.card-image-preview img');
    const urlInput = picker.querySelector('input[name="image"]');
    const fileInput = picker.querySelector('input[name="imageFile"]');

    const setMode = () => {
      const source = picker.querySelector('input[name="imageSource"]:checked')?.value || 'url';
      urlPanel?.classList.toggle('hidden', source !== 'url');
      filePanel?.classList.toggle('hidden', source !== 'file');
    };

    picker.querySelectorAll('input[name="imageSource"]').forEach((input) => {
    ...
    fileInput?.addEventListener('change', async () => {
      if (!preview || !previewImg || !fileInput.files?.[0]) return;
      try {
        previewImg.src = await readImageFileAsDataUrl(fileInput.files[0]);
...
function showPageLoading(text = 'جارٍ تجهيز حسابك...') {
  ...
  overlay.innerHTML = `
    <div class="page-loading-card glass">
      <span class="loader page-loading-spinner"></span>
      <strong>${escapeHtml(text)}</strong>
      <small>يتم تحديث بياناتك وتجهيز الألعاب الآن</small>
    </div>
  `;
...
async function initLogin() {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      isRedirecting = true;
      sessionStorage.setItem('fxzzLoginLoading', '1');
      showPageLoading('جارٍ تجهيز المكتبة...');
      setTimeout(() => {
        window.location.href = '/dashboard.html';
      }, 900);
    } catch (error) {
...
async function initDashboard() {
  const shouldShowLoginLoading = sessionStorage.getItem('fxzzLoginLoading') === '1';
  if (shouldShowLoginLoading) {
    showPageLoading('جارٍ تحديث مكتبتك...');
  }
  ...
  if (shouldShowLoginLoading) {
    sessionStorage.removeItem('fxzzLoginLoading');
...
function createFormModal({ title, subtitle = '', fields = [], submitText = 'حفظ التعديلات', onSubmit }) {
          ...
          ${fields.map((field) => `
            <div class="input-group ${field.full ? 'span-2' : ''}">
              ...
              ${field.type === 'textarea'
                ...
                : field.type === 'cardImage'
                  ? cardImagePickerMarkup({ value: field.value || '', inputName: field.name || 'image' })
                  : field.type === 'select'
  ...
  const modalImageGroup = root.querySelector('#dynamicModalForm input[name="image"]')?.closest('.input-group');
  if (modalImageGroup && !modalImageGroup.querySelector('[data-card-image-picker]')) {
    const currentImage = modalImageGroup.querySelector('input[name="image"]')?.value || '';
    modalImageGroup.innerHTML = `<label>صورة البطاقة</label>${cardImagePickerMarkup({ value: currentImage })}`;
  }
  setupCardImagePickers(root);
  $('#closeModalBtn').addEventListener('click', closeModal);
...
function renderCardsTab() {
  ...
  const imageInputGroup = createCardForm?.querySelector('input[name="image"]')?.closest('.input-group');
  if (imageInputGroup) {
    imageInputGroup.innerHTML = `<label>صورة البطاقة</label>${cardImagePickerMarkup()}`;
  }
  setupCardImagePickers(root);
  ...
  createCardForm.addEventListener('submit', async (event) => {
    ...
    const values = Object.fromEntries(formData.entries());
    try {
      values.image = await getCardImageValue(event.currentTarget);
      delete values.imageSource;
    ...
    button.addEventListener('click', () => {
      ...
      createFormModal({
        ...
        onSubmit: async (values, form) => {
          values.image = await getCardImageValue(form, card.image);
          delete values.imageSource;
          delete values.imageFile;
